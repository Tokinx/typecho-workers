...
**关键规则**：
- `admin:page` 是 `[slug].astro` 使用的 filter 风格注入点，但不属于 `HookPoints` 常量，因此不计入 6.6 的 70 个 Hook 点
