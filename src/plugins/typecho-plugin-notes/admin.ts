...
function renderList(append){var html=state.notes.map(renderNote).join("");if(append)list.insertAdjacentHTML("beforeend",html);else list.innerHTML=html||'<div class="notes-empty">当前筛选下没有笔记</div>';wireNotes();more.hidden=state.page>=state.totalPages;if(window.ViewImage)ViewImage.init("[view-image] img")}
...
async function openReference(cid){if(!Number.isSafeInteger(cid)||cid<1)return;referenceBody.textContent="正在加载笔记...";if(!referenceDialog.open)referenceDialog.showModal();try{var result=await api("/api/admin/notes?cid="+cid);var note=(result.data||[])[0];if(!note)throw new Error("笔记不存在");referenceBody.innerHTML='<article class="note-item"><div class="note-meta"><time>'+E(relative(note.created))+'</time></div><div class="note-body" view-image>'+(note.html||"")+'</div></article>';if(window.ViewImage)ViewImage.init("[view-image] img");referenceBody.querySelectorAll("[data-note-ref]").forEach(function(link){link.onclick=function(event){event.preventDefault();openReference(Number(link.dataset.noteRef||0))}})}catch(error){referenceBody.textContent=error.message||"加载笔记失败"}}
